...
*Authoritative as of the full applied migration set through `migrations/008_pivot_and_dependency_hardening.sql` (see the document header for revision history). PR #19's item-cap fix touched only query-layer behavior (`kt_render_roadmap`'s `_Generated` timestamp derivation, §3.13/§6.3 of `docs/TRD.md`), not the schema, so it has no entry in this appendix. An earlier draft of this appendix hand-copied a DDL block that had drifted substantially from the schema actually built — wrong table names, wrong columns, wrong nullability, and an entity (a database-backed auth-token store) that was never built at all. The Track and Decision rows below were updated (PR #16 review, T2.16) to drop the stored `tracks.status` column and add the `effect`/pivot-pointer columns migrations 006 and 007 introduced, then updated again for migration 008's composite-FK hardening. Several documented columns (`adapters.key_version`, `tracks.last_github_sync_at`/`last_linear_sync_at`, the pivot/effect columns) are added by migrations 004, 005, and 006, not present in 001 alone. `docs/DATABASE_SCHEMA.md` remains the canonical, authoritative reference for constraints, indexes, and design rationale; this is a summary for implementer convenience, not a second source of truth to keep in sync by hand.*
