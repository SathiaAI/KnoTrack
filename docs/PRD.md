...
**Business rules / edge cases:**
- **No initial status write (T2.16):** `kt_create_track` never writes a `status` value — there is no `tracks.status` column to write (§4.4). A brand-new track with no items reads `on_track` at the next read (an empty track is never `done`); if any `depends_on` track is not yet `effective_done`, the new track immediately reads `blocked` once queried, with no separate insert-time computation.
