...
*Full rewrite as of the 2026-09-07 reconciliation (see the document header): an earlier draft of this appendix hand-copied a DDL block that had drifted substantially from the schema actually built — wrong table names, wrong columns, wrong nullability, and an entity (a database-backed auth-token store) that was never built at all. The Track and Decision rows below were updated again (PR #16 review, T2.16) to drop the stored `tracks.status` column and add the `effect`/pivot-pointer columns migrations 006 and 007 introduced. The table overall matches the schema after the full applied migration set (`migrations/001_init.sql` through `migrations/007_resolves_decision_same_track.sql`) — several documented columns (`adapters.key_version`, `tracks.last_github_sync_at`/`last_linear_sync_at`, the pivot/effect columns) are added by migrations 004, 005, and 006, not present in 001 alone. A separate, not-yet-merged follow-up PR (migration 008) adds further hardening — a generated-column composite FK verifying a pivot pointer's target `effect`, and a `project_id` column on `track_dependencies` — not reflected below since it isn't part of this schema yet. `docs/DATABASE_SCHEMA.md` remains the canonical, authoritative reference for constraints, indexes, and design rationale; this is a summary for implementer convenience, not a second source of truth to keep in sync by hand.*
...
**Track** — **there is no `status` column (T2.16).** `id (uuid pk)`, `project_id (uuid fk)`, `title (text)`, `pivot_decision_id (uuid, nullable, composite fk -> decisions(id, track_id) — NULL means no active pivot, §4.4/§4.9)`, `source_doc_ref (text, nullable)`, `last_github_sync_at (timestamptz, nullable)`, `last_linear_sync_at (timestamptz, nullable — both added for the future SYNC_DRIFT rule, §4.11; not yet written by anything, since the sync tools aren't implemented)`, `created_at`, `updated_at`. `status` (`on_track|pivot_pending|blocked|done`) is computed at read time by the `track_readiness` database view from `pivot_decision_id`, the track's own item completion, and its direct dependencies' state — no column to write, so no tool writes it directly. Track-level dependencies: a `track_dependencies (track_id, depends_on_track_id, created_at)` join table, no self-dependency allowed. There is no `description` column. (A not-yet-merged follow-up PR further hardens `pivot_decision_id`'s FK to also verify the target decision's `effect`, via a generated column, and adds `project_id` to `track_dependencies` — neither is in this schema yet.)
...
`id (uuid pk)`, `project_id (uuid fk)`, `track_id (uuid fk, nullable — set to NULL by a hard track delete, preserving the Decision itself)`, `title (text)`, `rationale (text, NOT NULL)`, `what_changed (text, nullable — NOT NULL enforced only when effect is open_pivot/resolve_pivot)`, `effect (text, CHECK-constrained: note|open_pivot|resolve_pivot, default note — T2.16)`, `resolves_decision_id (uuid, nullable, self fk pinned to the same track via a composite fk on (resolves_decision_id, track_id) — migration 007 — set only on a resolve_pivot row, pointing at the open_pivot decision it resolves)`, `created_at`. There is no `item_ids` column and no `created_by` — Decisions in this build carry no per-caller identity, consistent with §5.3's shared-token auth model. (The same not-yet-merged follow-up PR mentioned in the Track row above also adds a generated `resolves_target_effect` column so this FK verifies the target's `effect` too, not just its track — not in this schema yet.)
