...
| `adapters` | object `{ github?, linear? }` | no | Per-adapter credentials, supplied directly in the call (not read from server env vars). `github: { personal_access_token, repo? }`; `linear: { api_key, team_id, done_state_id?, open_state_id?, label_id?, project_id? }` (the optional workflow-state overrides and the two tag targets are T5.3 — see §4.14). |
...
- Filtering, when `status` is supplied, is a direct `WHERE tr.status = ...` clause against the `track_readiness` view (§4.4 has the full note on track status being derived, not stored) — the view makes this a plain indexed-join filter, not a post-processing step.
...
3. Drop items whose track is not `dependenciesEffectivelyDone` — i.e. at least one **direct** dependency of the track is not `effective_done` (§4.4) — **not** a check against the track's own derived `status` (T2.16). This replaced an earlier "drop items whose track has status `blocked`" filter, which only ever caught a direct-dependency problem and missed one two-plus hops away; keying off `effective_done` closes that gap. A track can read `status: "on_track"` while still failing this check, and vice versa.
4. Order the survivors by: (a) track status priority — `on_track` before `pivot_pending` (a survivor's track can never actually be `blocked`, since step 3 already drops those); (b) `sequence_position` ascending; (c) `created_at` ascending; (d) `id` ascending as the final, fully deterministic tie-break — needed because track priority, `sequence_position`, and `created_at` can all still tie (e.g. two items created in the same millisecond).
...
**Output:** `{ track_id, warnings? }` — `warnings` (an array of strings) is present only when at least one `depends_on` track isn't yet `effective_done`; omitted entirely otherwise.

**Business rules / edge cases:**
- **No initial status write (T2.16):** `kt_create_track` never writes a `status` value — there is no `tracks.status` column to write (§4.4). A brand-new track with no items reads `on_track` at the next read (an empty track is never `done`) — unless a `depends_on` track is itself not `own_done`, or has an active pivot, in which case the new track immediately reads `blocked` once queried, with no separate insert-time computation. This is `blocked`'s ordinary one-hop check (§4.4) — it looks only at each `depends_on` track's own local state, never at that track's `effective_done`. **The `warnings` field below uses a different, stricter check (`effective_done`, not `own_done`)** — so it's possible for creation to produce a `warnings` entry for a `depends_on` track that is itself `own_done` (locally complete) but not `effective_done` (broken two-plus hops away), while the new track still reads `on_track`, not `blocked`. The two checks answer different questions and can disagree.
- **Dependency completeness is a warning, not a hard gate.** If any track listed in `depends_on` is not currently `effective_done`, creation still succeeds — the response includes a `warnings: string[]` field naming the not-yet-complete dependency ids. Blocking creation on an incomplete dependency would make it impossible to pre-declare a track's place in a plan before its prerequisites finish, which is a normal and expected planning sequence. `warnings` is omitted (not an empty array) when there is nothing to warn about.
...
**Description:** Record an explicit, human/agent-authored pivot or decision. Never inferred — a Decision only exists because this tool was called with real rationale text. Whether recording one changes the track's pivot state at all, and how, is controlled by the `effect` input (T2.16) — it no longer unconditionally pivots the track.

**Inputs:** `project_id` (required), `track_id` (**required**), `title` (required, 1–300 chars), `rationale` (required, 1–5000 chars, non-empty), `what_changed` (required, 1–5000 chars, non-empty — concrete description, e.g. "Track B reprioritized ahead of Track A because the client moved up the Track B deadline"), `effect` (optional enum `note | open_pivot | resolve_pivot`, default `note`), `expected_pivot_decision_id` (optional uuid — required if and only if `effect: "resolve_pivot"`, rejected with `VALIDATION_ERROR` if supplied with any other `effect`). There is no `item_ids` input — a Decision is scoped to a track, not to individual items, in v1.

**Output:** `{ decision_id }`

**Business rules / edge cases:**
- `rationale` or `what_changed` being empty → `VALIDATION_ERROR`, for every `effect` including `note` — the tool's input schema requires both unconditionally (this is the entire point of the entity: a Decision must carry real explanatory content, never a bare boolean). This is stricter than the database column itself, which only requires `what_changed` to be non-NULL for `open_pivot`/`resolve_pivot` (Appendix §9's Decision row) — that DB-level looseness exists as defense-in-depth against some future writer other than this tool, not because this tool ever omits it.
- **Effect on the track's pivot pointer (T2.16) — a plain `"note"` (the default) has no side effect at all.** Only `open_pivot` and `resolve_pivot` touch `tracks.pivot_decision_id`, both as compare-and-set operations guarded against races, in the same transaction as inserting the Decision row:
  - `effect: "note"` — inserts the Decision row only. No change to the track's pivot state. This replaced an earlier design (this document's own pre-T2.16 draft) where every Decision set the track to `pivot_pending`; a Decision that's just a record of reasoning, with no open question attached, is common and shouldn't force a pivot.
  - `effect: "open_pivot"` — inserts the Decision row, then sets `tracks.pivot_decision_id` to this new Decision's id, guarded by "only if there is no active pivot already." A track that already has an active pivot gets `CONFLICT`, not a silent overwrite.
  - `effect: "resolve_pivot"` — requires `expected_pivot_decision_id` (the id the caller last read as the track's active pivot, via `kt_get_track` — `kt_list_tracks`'s output does not include `pivot_decision_id`, §4.3). Clears `tracks.pivot_decision_id`, guarded by "only if it currently equals `expected_pivot_decision_id`," and only inserts the Decision row (with a reference back to the resolved pivot) once that clear actually took effect. **This is the write path that moves a track back out of `pivot_pending`** — this document previously described that as an open gap (PR-review Finding 2); T2.16 closes it. Two distinct `CONFLICT` cases, both `409`: no active pivot at all, or the active pivot is a different Decision than the caller expected (someone else already resolved or reopened it) — either way nothing is overwritten silently.
...
- **Given** track T is currently `on_track`, **when** a Decision is recorded for T with `effect: "open_pivot"`, **then** T's `status` becomes `pivot_pending` (verifiable via `kt_get_track` or `kt_list_tracks`).
- **Given** track T is currently `on_track`, **when** a Decision is recorded for T with the default `effect: "note"`, **then** T's `status` is unchanged.
- **Given** track T has an active pivot opened by Decision D, **when** a Decision is recorded for T with `effect: "resolve_pivot"` and `expected_pivot_decision_id: D`, **then** T's `pivot_decision_id` clears and T's `status` is no longer `pivot_pending` (assuming no other reason it would be `blocked`).
...
|---|---|---|
| `STALE_TRACK` | `warning` | Track's derived status (§4.4, T2.16 — computed at read time, not stored) is `on_track` and no session-summary Event on that track has `created_at` within `KNOTRACK_STALE_TRACK_DAYS` (default 14) days; if the track has zero Events ever, measured from the track's `created_at` instead. |
...
| `SYNC_DRIFT` | `warning` | The project has credentials configured for an adapter, and a track's most recent activity is later than that adapter's last successful sync of it. **Schema exists (`tracks.last_github_sync_at`/`last_linear_sync_at`); the `SYNC_DRIFT` rule itself and the `kt_check_drift` scan are not yet built (T6).** `kt_sync_to_github` (§4.13, T5.2) and `kt_sync_to_linear` (§4.14, T5.3) are both implemented and write `last_github_sync_at` / `last_linear_sync_at` respectively on each successful sync. |
...
**Description:** **Implemented (T5.2, 2026-09-19 — replaces the earlier T2.13 stub slice).** One GitHub sync operation, scoped to a single track, per call — there is no `pull`/`push`/direction choice and no item-level sync target in v1. It creates a GitHub Issue for the track on first sync and updates it on subsequent syncs, idempotently: the authoritative "does an issue already exist for this track" record is a `track_external_links` row (`migrations/009`), never a GitHub search. Idempotency is hardened with a durable creation-intent — a `pending` link row is committed *before* the outbound create, so a crash between GitHub accepting the create and the server recording the result never silently produces a duplicate (the next call recovers via a hidden `<!-- knotrack:track:<id> -->` body marker). The created/updated issue's `html_url` is recorded on the track and surfaced on `kt_get_track` as `track.github_issue_url` (that is what "records the issue URL on the track" means — there is no separate URL column on `tracks`). Retries/backoff are intentionally out of scope for v1: a transient upstream failure surfaces as `{ ok: false }` for the caller to retry.

**Inputs:** `project_id` (required), `track_id` (required). There is no `direction`, `github_repo` override, or `item_id` input.

**Output (by outcome):**
- Success: `{ ok: true }` (whether the issue was created, updated, or was already up to date — a no-op re-sync makes no HTTP call).
...
**Business rules / edge cases:**
- **Two distinct failure surfaces, deliberately kept separate.** A precondition the caller can fix by configuring the adapter first — no GitHub credentials stored for this project (no row in `adapters` for `(project_id, "github")`), **or** a stored GitHub adapter with no `repo` configured (so no API URL can be formed) — is a real tool-level error, `CONFLICT`, via the standard error envelope, and makes no HTTP call. Everything about actually talking to GitHub itself is **not** an MCP-level error at all — the call succeeds and returns the discriminated `{ok: false, error: ...}` result instead, because these are expected, retryable operational outcomes, not contract violations.
- **Idempotency / no duplicates.** The target repo is read from the stored adapter config, never a tool argument. Repeated syncs of the same track never create a second Issue: create-vs-update is decided from the `track_external_links` row, a re-sync with no content change is a no-op (no HTTP call), and the link is bound to the exact `owner/repo` it was created against — a later silent change to the adapter's repo makes the sync refuse rather than update an unrelated issue. A stale-link `404` on update returns `GITHUB_NOT_FOUND` and does **not** auto-recreate.
...
**Acceptance criteria:**
- **Given** a project without a GitHub adapter configured (or a GitHub adapter with no `repo`), **when** `kt_sync_to_github` is called, **then** the call fails with `CONFLICT` and no HTTP call is made.
- **Given** a configured GitHub adapter, **when** `kt_sync_to_github` is called for a track with no existing link, **then** an Issue is created, the link is recorded, and `kt_get_track`'s `track.github_issue_url` returns the Issue URL; a second call with unchanged content is a no-op (no HTTP call) and a call after the track changes updates the same Issue.
- **Given** GitHub returns a 403 rate-limit response, **when** `kt_sync_to_github` is called, **then** the response is `{ ok: false, error: "GITHUB_RATE_LIMITED: ..." }` — a successful tool call, not a thrown error.
- **Given** a `pending` link left by a crashed prior create, **when** `kt_sync_to_github` is called again, **then** it recovers via the hidden marker (adopting the existing Issue if found) rather than creating a duplicate. *Verified offline (fake `GitHubClient`, no network in CI); the real-repo run against `SathiaAI/KnoTrack` with a fine-grained PAT is the one remaining acceptance gate (docs/ROADMAP.md T5.2).*

### 4.14 `kt_sync_to_linear`

**Description:** Identical shape and semantics to `kt_sync_to_github` (§4.13), mirrored for Linear. As of T5.3 it ships the full operational Linear sync path — create/update a linked Linear Issue for a track, idempotently, over the Linear GraphQL API — with the same durable creation-intent and hidden-marker crash recovery as §4.13. The one Linear-specific difference is workflow state: Linear has no open/closed, so a done track's issue is moved to a `completed` workflow state (the team's lowest-position `completed` state, or an optional configured `done_state_id`), and a non-done track's issue is never auto-moved backward unless a valid `open_state_id` is configured (both optional fields on the Linear adapter config). Every synced Issue is also stamped with a configured KnoTrack **label** (`label_id`) and **project** (`project_id`) when set, so KnoTrack-managed Issues are distinguishable in a shared workspace; both must reference pre-existing Linear resources — the sync never auto-creates them, and a missing/mismatched id returns `LINEAR_TAG_CONFIG`. The label and project are applied on every create and every content-changing update; they are **not** re-enforced on a no-op sync (unchanged content), so if a user manually removes the label or the project from a synced Issue it is restored on the track's next content change, not on the next poll.

**Inputs:** `project_id` (required), `track_id` (required).

**Output:** `{ ok: true }` on success; `{ ok: false, error: "<PREFIX>: <detail>" }` on an operational failure. Prefixes: `LINEAR_AUTH_FAILED`, `LINEAR_NOT_FOUND`, `LINEAR_RATE_LIMITED`, `LINEAR_TIMEOUT` (exceeds `KNOTRACK_LINEAR_SYNC_TIMEOUT_MS`, default 8000ms), `LINEAR_STATE_CONFIG` (an invalid `done_state_id`/`open_state_id`, or a done track whose team has no `completed` workflow state and no `done_state_id` configured), `LINEAR_TAG_CONFIG` (a configured `label_id`/`project_id` that does not exist or does not belong to the team), `LINEAR_UNKNOWN_ERROR`.

**Business rules / edge cases (mirrors §4.13 exactly, Linear-specific specifics only):**
- `CONFLICT` for missing Linear credentials (no row in `adapters` for `(project_id, "linear")`) or a Linear adapter with no `team_id`; everything about the Linear API call itself is a non-error `{ok: false, ...}` result.
- Workflow-state mapping: a done track's Issue is moved to a `completed` state (the configured `done_state_id`, else the team's lowest-position `completed` state); an invalid override, or a done track whose team has no `completed` state and no `done_state_id`, returns `{ok:false, error:"LINEAR_STATE_CONFIG: ..."}`. A non-done track's Issue is never moved backward unless a valid `open_state_id` is configured and (on update) the Issue is currently in a `completed`/`canceled` state.

**Acceptance criteria:**
- **Given** a project without Linear credentials configured, **when** `kt_sync_to_linear` is called, **then** the call fails with `CONFLICT`.
- **Given** a configured Linear adapter, **when** `kt_sync_to_linear` is called for a track with no existing link, **then** a Linear Issue is created, the link is recorded, and `kt_get_track`'s `track.linear_issue_url` returns the Issue URL; a second call with unchanged content is a no-op and a call after the track changes updates the same Issue. A done track's Issue is set to a `completed` workflow state.
- **Given** a `pending` link left by a crashed prior create, **when** `kt_sync_to_linear` is called again, **then** it recovers via the hidden marker (adopting the existing Issue if found) rather than creating a duplicate. *Verified offline (fake `LinearClient`, no network in CI); the real-workspace run is the one remaining acceptance gate (docs/ROADMAP.md T5.3).*
...
*Authoritative as of the full applied migration set through `migrations/008_pivot_and_dependency_hardening.sql` (see the document header for revision history). PR #19's item-cap fix touched only query-layer behavior (`kt_render_roadmap`'s `_Generated` timestamp derivation, §3.13/§6.3 of `docs/TRD.md`), not the schema, so it has no entry in this appendix. An earlier draft of this appendix hand-copied a DDL block that had drifted substantially from the schema actually built — wrong table names, wrong columns, wrong nullability, and an entity (a database-backed auth-token store) that was never built at all. The Track and Decision rows below were updated (PR #16 review, T2.16) to drop the stored `tracks.status` column and add the `effect`/pivot-pointer columns migrations 006 and 007 introduced, then updated again for migration 008's composite-FK hardening. Several documented columns (`adapters.key_version`, `tracks.last_github_sync_at`/`last_linear_sync_at`, and migration 006's original pivot-pointer/effect columns — `tracks.pivot_decision_id`, `decisions.effect`) are added by migrations 004, 005, and 006, not present in 001 alone; migration 008 later added its own generated effect-verification columns — `tracks.pivot_effect`, `decisions.resolves_target_effect` — described in the Track and Decision rows below, not to be confused with migration 006's `decisions.effect`. `docs/DATABASE_SCHEMA.md` remains the canonical, authoritative reference for constraints, indexes, and design rationale; this is a summary for implementer convenience, not a second source of truth to keep in sync by hand.*

Track- and item-level dependencies are stored as **join tables** (`track_dependencies(track_id, depends_on_track_id, project_id, created_at)`, `item_dependencies(item_id, depends_on_item_id, created_at)`), not array columns — despite `depends_on_track_ids`/`depends_on_item_ids` appearing as arrays in every tool's JSON input/output (§4), that shape is assembled at the API layer from the join tables, not stored directly on `tracks`/`items`. (`track_dependencies.project_id` was added by migration 008 — see the Track row below.)
...
**Track** — **there is no `status` column (T2.16).** `id (uuid pk)`, `project_id (uuid fk)`, `title (text)`, `pivot_decision_id (uuid, nullable, composite fk -> decisions(id, track_id, effect) — NULL means no active pivot, §4.4/§4.9)`, `pivot_effect (text, GENERATED ALWAYS AS ('open_pivot') STORED — migration 008)` — exists only so `pivot_decision_id`'s FK can be a 3-column composite that also pins the referenced decision's `effect` to `open_pivot`, not just its identity/track, `source_doc_ref (text, nullable)`, `last_github_sync_at (timestamptz, nullable)`, `last_linear_sync_at (timestamptz, nullable — both added for the SYNC_DRIFT rule, §4.11; `last_github_sync_at` is now written by `kt_sync_to_github` on each successful sync (T5.2) and `last_linear_sync_at` by `kt_sync_to_linear` (T5.3), each stamped to the payload snapshot time)`, `created_at`, `updated_at`. `status` (`on_track|pivot_pending|blocked|done`) is computed at read time by the `track_readiness` database view from `pivot_decision_id`, the track's own item completion, and its direct dependencies' state — no column to write, so no tool writes it directly. Track-level dependencies: a `track_dependencies (track_id, depends_on_track_id, project_id, created_at)` join table, no self-dependency allowed; `project_id` (migration 008, backfilled from `tracks.project_id`, NOT NULL) is enforced by two composite FKs to `tracks(id, project_id)` so both sides of an edge are schema-guaranteed to belong to the same project — closing a gap that was previously only enforced at the application layer (`kt_create_track`). There is no `description` column. Also unique on `(id, project_id)` (migration 008, backing those composite FKs).
...
`id (uuid pk)`, `project_id (uuid fk)`, `track_id (uuid fk, nullable — set to NULL by a hard track delete, preserving the Decision itself)`, `title (text)`, `rationale (text, NOT NULL)`, `what_changed (text, nullable — NOT NULL enforced only when effect is open_pivot/resolve_pivot)`, `effect (text, CHECK-constrained: note|open_pivot|resolve_pivot, default note — T2.16)`, `resolves_decision_id (uuid, nullable, self fk pinned to the same track and effect via a 3-column composite fk on (resolves_decision_id, track_id, resolves_target_effect) -> decisions(id, track_id, effect) — migration 008, superseding migration 007's same-track-only version — set only on a resolve_pivot row, pointing at the open_pivot decision it resolves)`, `resolves_target_effect (text, GENERATED ALWAYS AS (CASE WHEN resolves_decision_id IS NOT NULL THEN 'open_pivot' END) STORED — migration 008)` — same trick as `pivot_effect` above: `NULL` when `resolves_decision_id` is `NULL`, `'open_pivot'` otherwise, so the composite FK verifies the target decision's `effect` too, not just its track, `created_at`. Also unique on `(id, track_id, effect)` (migration 008, backing this row's and Track's composite FKs). This FK uses the default `MATCH SIMPLE`, not `MATCH FULL` — an earlier draft of migration 008 tried `MATCH FULL` to close a real gap (a `resolve_pivot` row with a NULL `track_id` bypassing the FK check entirely under `MATCH SIMPLE`), but `MATCH FULL` rejects any row mixing NULL and non-NULL key columns, which would have broken every ordinary `note`/`open_pivot` decision (`resolves_decision_id` and `resolves_target_effect` both NULL, `track_id` non-NULL — already a mixed-NULL row) — Postgres raises "MATCH FULL does not allow mixing of null and nonnull key values" for exactly that shape. The corrected fix keeps `MATCH SIMPLE` and closes the actual gap with a `BEFORE INSERT OR UPDATE OF effect, resolves_decision_id, track_id` trigger (`trg_decisions_track_id_required_for_pivots`) — not every `UPDATE`, only one that touches one of those three columns — that rejects any non-`note` decision row with a NULL `track_id`, except during the legitimate hard-track-delete `ON DELETE SET NULL` cascade. There is no `item_ids` column and no `created_by` — Decisions in this build carry no per-caller identity, consistent with §5.3's shared-token auth model.
