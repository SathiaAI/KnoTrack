...
**Track** — **there is no `status` column (T2.16).** `id (uuid pk)`, `project_id (uuid fk)`, `title (text)`, `pivot_decision_id (uuid, nullable, composite fk -> decisions(id, track_id, effect) — NULL means no active pivot, §4.4/§4.9)`, `pivot_effect (text, GENERATED ALWAYS AS ('open_pivot') STORED — migration 008)` — exists only so `pivot_decision_id`'s FK can be a 3-column composite that also pins the referenced decision's `effect` to `open_pivot`, not just its identity/track, `source_doc_ref (text, nullable)`, `last_github_sync_at (timestamptz, nullable)`, `last_linear_sync_at (timestamptz, nullable — both added for the future SYNC_DRIFT rule, §4.11; not yet written by anything, since the sync tools aren't implemented)`, `created_at`, `updated_at`. `status` (`on_track|pivot_pending|blocked|done`) is computed at read time by the `track_readiness` database view from `pivot_decision_id`, the track's own item completion, and its direct dependencies' state — no column to write, so no tool writes it directly. Track-level dependencies: a `track_dependencies (track_id, depends_on_track_id, project_id, created_at)` join table, no self-dependency allowed; `project_id` (migration 008, backfilled from `tracks.project_id`, NOT NULL) is enforced by two composite FKs to `tracks(id, project_id)` so both sides of an edge are schema-guaranteed to belong to the same project — closing a gap that was previously only enforced at the application layer (`kt_create_track`). There is no `description` column. Also unique on `(id, project_id)` (migration 008, backing those composite FKs).
