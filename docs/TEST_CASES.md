...
| GHSY-18 | kt_sync_to_github | Recovery (no duplicates) | A `pending` link left by a crashed prior (ambiguous) create | Same call | Recovers via the hidden `<!-- knotrack:track:<id> -->` marker: **adopts** the existing Issue when the marker is found; on a miss it **never** auto-recreates (a Search miss can't prove the ambiguous create didn't land) — it returns an actionable error and leaves the pending link for explicit resolution. Duplicate prevention takes priority over auto-retry. |
