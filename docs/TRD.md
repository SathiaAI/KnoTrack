...
- **Railway:** `DATABASE_URL` is auto-injected into the app's environment when a Postgres plugin is attached in the same project — no manual wiring needed. Migrations run via a `startCommand` wrapper: `node dist/scripts/migrate.js && node dist/src/index.js` — **not** `npm run migrate && npm start`: `npm run migrate` invokes `tsx scripts/migrate.ts`, and `tsx` is a devDependency the production Docker image deliberately omits (`npm ci --omit=dev`, see `Dockerfile`'s header comment), so that command fails in the actual deployed container even though it works locally. `npm start` itself is fine (`node dist/src/index.js` already), but pairing it with the wrong migrate invocation would still break the combined `startCommand`. See `docs/deploy/railway.md`.
