...
**The authoritative schema is `migrations/001_init.sql`** (plus `002_projects_unique_source_ref.sql`, `003_drift_flags_open_unique.sql`, `004_adapters_key_version.sql`, `005_tracks_sync_timestamps.sql`, `006_derived_track_status.sql`, and `007_resolves_decision_same_track.sql`), applied by the custom runner at `scripts/migrate.ts` (§1). It is not reproduced here.
