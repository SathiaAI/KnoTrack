...
Tool files — `record-decision.ts` and `update-item-status.ts` are now real, dedicated files too (T2 build-out, second slice, 2026-08-26), joining `list-tracks.ts`, `get-track.ts`, `get-next-steps.ts`, and `render-roadmap.ts` from the first slice. 11 of the 14 tools listed above are real files; the remaining 3 (`check-drift.ts`, `sync-to-github.ts`, `sync-to-linear.ts`) are still registered together in one `stubs.ts` file (so `src/mcp/tools/` currently has 12 files on disk, not 14 — the 11 real ones plus `stubs.ts`). `src/db/queries/decisions.ts` now exists (shipped alongside `record-decision.ts`). `src/adapters/` still doesn't exist at all — no code path uses it until `T5` starts.
