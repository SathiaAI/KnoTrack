...
**This tree describes the target layout, not yet the current one on two fronts (updated 2026-09-07 — see `docs/ROADMAP.md`'s T2/T3 status for the underlying build state):**

Tool files — `record-decision.ts` and `update-item-status.ts` are now real, dedicated files too (T2 build-out, second slice, 2026-08-26), joining `list-tracks.ts`, `get-track.ts`, `get-next-steps.ts`, and `render-roadmap.ts` from the first slice. All 14 tools listed above are real files: `check-drift.ts`, `sync-to-github.ts`, and `sync-to-linear.ts` each got their own file (2026-09-18, PR #22), and the old `stubs.ts` was removed. `src/db/queries/decisions.ts` now exists (shipped alongside `record-decision.ts`). The T5 sync work has since shipped: the real adapter clients live in `src/github/github-client.ts` (T5.2) and `src/linear/linear-client.ts` (T5.3), and the interim `sync-shared.ts` stub helper was removed once both sync tools shipped their real implementations. `src/adapters/` was never created.

Deploy config — `render.yaml`, `railway.toml`, and `fly.toml` do **not** exist in this repo and never have; only `Dockerfile` does. Railway is configured directly through its own dashboard/API rather than a committed `railway.toml` (see `docs/deploy/railway.md`), and this is the only one of the three deploy targets that has actually been built, deployed, and verified (`docs/ROADMAP.md` T3, done as of 2026-09-05) — Render+Supabase and Fly.io are undeployed, unverified target designs (`docs/ROADMAP.md` T7.1/T7.2, both still `blocked`). §7 below describes all three platforms' migration mechanics for planning purposes, but only the Railway entry reflects a real, exercised deployment.
...
**No initial status write (T2.16):** `status` is derived, not stored (§3.5), so `kt_create_track` never writes it. A brand-new track with no items reads `on_track` (an empty track is never `done`) — unless a `depends_on` track is itself not `own_done`, or has an active pivot, in which case the new track immediately reads `blocked` once the view is queried. This is `blocked`'s ordinary one-hop check (§3.5) — it looks only at each `depends_on` track's own local state (`own_done`/`has_pivot`), never at that track's `effective_done`. The `warnings` field below uses a different, stricter check (`effective_done`, not `own_done`) — so it's possible for creation to produce a `warnings` entry for a `depends_on` track that is itself `own_done` (locally complete) but not `effective_done` (broken two-plus hops away), while the new track still reads `on_track`, not `blocked`. The two checks answer different questions and can disagree.
...
- `effect: "resolve_pivot"` — requires `expected_pivot_decision_id` (the id the caller last read as the track's active pivot). Clears `tracks.pivot_decision_id` first, guarded by `WHERE pivot_decision_id = $expected_pivot_decision_id`, and only inserts the decision row (with `resolves_decision_id` set to that id) once that UPDATE actually affects a row — guarded-UPDATE-before-INSERT, not the other way around, precisely so two concurrent resolvers racing on the same pivot can't both reach the INSERT (PR #16 Codex review; `decisions_resolves_decision_id_uq` would otherwise turn the loser's raw unique-constraint violation into an unhandled `500` instead of the `409` below). Two ways this can fail, both `409 CONFLICT`, distinguished in the error message: the track has no active pivot at all ("no active pivot to resolve"), or the track's active pivot is a *different* decision than the one the caller expected ("pivot changed since you last read this track" — e.g. a concurrent caller already resolved it and a new one may have opened since). Either way nothing is overwritten silently.
...
- **Never returned:** the `adapters.config` column stores **only non-secret metadata** — e.g. `{"owner": "acme", "repo": "widgets"}` for GitHub, `{"team_id": "..."}` for Linear. (There is no `projects.adapters` column; an earlier draft of this section described one, but it was never part of the migrated schema — see `docs/DATABASE_SCHEMA.md`'s `projects` table.) As of this build, `kt_get_project_status`, `kt_list_tracks`, and `kt_get_track` don't yet serialize any adapter data into their responses at all — none of the three currently reads the `adapters` table. If/when they do, they must read only `config`, never `encrypted_credential`. As of T5.2/T5.3 the sync path is implemented: `kt_sync_to_github`/`kt_sync_to_linear` read the adapter row via `getAdapterForProject` (`src/db/queries/adapters.ts`) and decrypt it with `decryptCredential` (`src/crypto/credential-cipher.ts`) immediately before the outbound API call, then pass the plaintext token only to the injectable client — the `encrypted_credential` column never appears in any tool's output type. The real clients live at `src/github/github-client.ts` (T5.2) and `src/linear/linear-client.ts` (T5.3); there is no `src/adapters/` directory. `scripts/rotate-encryption-key.ts` reads every row via `listAllAdapters` (`src/db/queries/adapters.ts`) — the only credential reader outside the sync path. (`listAdaptersForProject`, a project-scoped `SELECT *` helper in the same file, is currently unused.)
...
- **Credential revocation (T5.4):** revoking a single leaked adapter credential is a separate operator action from key rotation — `scripts/revoke-credential.ts` (`npm run revoke-credential <project_id> <github|linear>`, or `node dist/scripts/revoke-credential.js ...` in the runtime image) calls `deleteAdapterForProject` to delete that project's `adapters` row. There is no revocation MCP tool (the mandated 14-tool set has none; `kt_register_project` is the only credential-mutation tool, and it can rotate but not remove). The script previews by default and requires `--yes` to delete (a fat-finger guard), prints an audit line (timestamp, operator, project, type, rows_deleted), and reminds the operator that this removes KnoTrack's stored copy only — the token must also be revoked provider-side. After revocation the sync-tool precondition (`getAdapterForProject`/`adapterConfigured`) sees no row, so any sync that STARTS after the delete commits returns `CONFLICT: adapter not configured`; the credential is decrypted per-call, so a revoked token is never served from a cache. (A sync already in flight may finish with the token it read before the delete — the revocation takes effect from the next sync; to quiesce, pause the client's polling before revoking.)
...
- `statement_timeout` is also set on the main pool, from `KNOTRACK_DB_STATEMENT_TIMEOUT_MS` (default **30000**) — Postgres itself cancels any single query running longer than this, so a slow or deadlocked query can't hang a request (and the connection it holds) indefinitely. *Added after this section was first written — see §7 for the full env var entry.*
- **A second, separate pool exists** for `/health` checks (`src/server/health-route.ts`): `max: 2`, `connectionTimeoutMillis: 2000`, `idleTimeoutMillis: 10000`, and its own `statement_timeout` hardcoded to **1000ms** — independent of `KNOTRACK_DB_STATEMENT_TIMEOUT_MS`, which applies only to the main pool above. This isolates `/health`'s blast radius from tool traffic, but means the process's real peak connection count is `KNOTRACK_DB_POOL_MAX + 2`, not just `KNOTRACK_DB_POOL_MAX` — operators provisioning to the connection ceiling should account for the extra 2.
...
| `KNOTRACK_DB_STATEMENT_TIMEOUT_MS` | Optional | `30000` | `statement_timeout` set on the main connection pool only (§6.2) — Postgres cancels any single query that runs longer than this, so a slow or deadlocked query can't hang a request indefinitely. Does **not** apply to the separate `/health`-check pool, which hardcodes its own 1000ms `statement_timeout` regardless of this variable. |
...
**Deploy-target quirks summary** (all three run the identical codebase/schema — no target-specific branches in application code, only environment-variable configuration). **Verification status differs sharply between these three, which the bullets below don't distinguish on their own:** only **Railway** has actually been deployed and verified end-to-end (`docs/ROADMAP.md` T3, done as of 2026-09-05, plus a real second-client smoke test in T4) and has a real, incident-tested runbook (`docs/deploy/railway.md`). **Render and Fly.io are undeployed, unverified target designs** — their mechanics below are inferred from each platform's own documentation, not exercised against a running KnoTrack instance, and neither has an *executed* `docs/deploy/` runbook yet (`docs/ROADMAP.md` T7.1/T7.2, both still `blocked`) — a draft covering both platforms already exists at `docs/deploy/client-verification-runbook.md`, but by its own header nothing in it is marked done until it's actually been run against a live deployment. Treat the Render and Fly.io bullets as a starting point to verify, not a confirmed-working configuration:

- **Render (free tier):** no persistent local disk — this is *the* reason Postgres-only is mandated (§1). Requires `GET /health` (§8) configured as Render's health-check path, since free-tier instances spin down on idle and Render polls this path to detect when the cold-started instance is ready again; the first request after a cold start may exceed the §6.1 budgets and this is an accepted, documented exception rather than a bug. **Migration mechanism differs by plan (adversarial PR review finding: this previously said unconditionally "via Render's Pre-Deploy Command," but Render's own docs restrict Pre-Deploy Command to paid web services/private services/background workers — it does not exist as an option on the Free instance type at all, not merely "requires upgrading" as a soft recommendation).** On a **paid** Render plan, migrations run via Render's Pre-Deploy Command, set to `node dist/scripts/migrate.js` (not `npm run migrate` — see the Railway note below for why the compiled form is required); Start Command remains `node dist/src/index.js` and does not run migrations itself. On the **Free** plan, where Pre-Deploy Command isn't available, Start Command instead does both, sequentially: `node dist/scripts/migrate.js && node dist/src/index.js` — safe specifically because a Free-tier web service runs exactly one instance with no autoscaling, so there is no concurrent-invocation race for `scripts/migrate.ts`'s advisory lock (§1) to even need to arbitrate, and `src/index.ts`'s own fail-loud schema guard (`docs/ROADMAP.md` `T3.8`) still refuses to serve traffic if the chained command's migration step failed silently for any reason. This is a deliberate exception to this document's general preference for migrations as a step separate from the app process (stated in §8 below) — it is what the Free plan's own restrictions leave available, not a stance that separate-step is unnecessary elsewhere.
...
**The authoritative schema is `migrations/001_init.sql`** (plus `002_projects_unique_source_ref.sql`, `003_drift_flags_open_unique.sql`, `004_adapters_key_version.sql`, `005_tracks_sync_timestamps.sql`, `006_derived_track_status.sql`, `007_resolves_decision_same_track.sql`, and `008_pivot_and_dependency_hardening.sql`), applied by the custom runner at `scripts/migrate.ts` (§1). It is not reproduced here.
