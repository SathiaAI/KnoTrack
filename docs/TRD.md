...
Tool files — `record-decision.ts` and `update-item-status.ts` are now real, dedicated files too (T2 build-out, second slice, 2026-08-26), joining `list-tracks.ts`, `get-track.ts`, `get-next-steps.ts`, and `render-roadmap.ts` from the first slice. All 14 tools listed above are real files: `check-drift.ts`, `sync-to-github.ts`, and `sync-to-linear.ts` each got their own file (2026-09-18, PR #22, with the two sync tools sharing `sync-shared.ts`), and the old `stubs.ts` was removed. `src/db/queries/decisions.ts` now exists (shipped alongside `record-decision.ts`). `src/adapters/` still doesn't exist at all — no code path uses it until `T5` starts.
