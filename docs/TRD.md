...
| `KNOTRACK_DB_STATEMENT_TIMEOUT_MS` | Optional | `30000` | `statement_timeout` set on every pooled connection (§6.2) — Postgres cancels any single query that runs longer than this, so a slow or deadlocked query can't hang a request indefinitely. |
