...
**Pre-existing-cycle check (Codex automated review of this PR):** replacing
`reject_track_dependency_cycle()` only protects mutations from that point forward — if
the snapshot-isolation race above had already been exploited against a live deployment
before migration 008 ran, the graph could already contain a cycle that the fixed
function alone would never notice. Migration 008 opens with a `DO $$ ... $$` block
(run before any of its other changes) that walks the same reachability CTE across every
existing edge and aborts the whole migration with `SQLSTATE 23514` and a remediation
message naming the offending track if any cycle is found — there is no safe way for the
migration to guess which edge to remove, so it stops and asks a human to.
...
- `trg_decisions_track_id_required_for_pivots` (`BEFORE INSERT OR UPDATE OF effect, resolves_decision_id, track_id` trigger, backed by `reject_pivot_decision_without_track()`) — rejects any row with `effect <> 'note'` and a `NULL` `track_id`. Closes the actual gap Astra's panel review identified for finding 2 (a `resolve_pivot` row with a `NULL` `track_id` would bypass the composite FK above under its default `MATCH SIMPLE`) — **not** via `MATCH FULL` as originally proposed, and **not** via a plain `CHECK` constraint either (the migration's first draft). `MATCH FULL` was verified, while implementing this migration, to reject every ordinary `'note'` decision that has a non-`NULL` `track_id` (the overwhelming majority of the table): Postgres's actual `MATCH FULL` rule is "no row may mix `NULL` and non-`NULL` key columns," not "skip unless every column is populated," and a plain note's `resolves_decision_id`/`resolves_target_effect` (`NULL`) alongside a normal `track_id` (non-`NULL`) is exactly that mixed shape. A `CHECK` constraint was then verified, in response to Codex's automated review of this same PR, to break the documented track-hard-delete/legal-erasure path: `decisions.track_id ... ON DELETE SET NULL` (see the `decisions` ON DELETE reasoning below) re-validates every `CHECK` on the row it nulls out, so any decision that ever recorded an `open_pivot`/`resolve_pivot` would make its own track undeletable. A trigger avoids that, but an `INSERT`-only one left two later-found gaps: a raw `UPDATE` flipping an existing trackless `'note'` to `'open_pivot'`/`'resolve_pivot'` skipped the guard entirely (CodeRabbit re-review — closed by adding `UPDATE OF effect, resolves_decision_id`), and a raw `UPDATE decisions SET track_id = NULL` on a decision no longer pointed to by `tracks.pivot_decision_id` (e.g. an already-resolved pivot) wasn't caught by anything else in the schema either (Codex automated review — closed by adding `track_id` to the same `UPDATE OF` list). The function distinguishes that direct write from the legitimate `ON DELETE SET NULL` cascade by checking, only on `UPDATE`, whether `OLD.track_id`'s row in `tracks` still exists — gone means the cascade already deleted it (allowed); still present means a direct write (rejected) — so hard-deleting a track with a live pivot decision still succeeds exactly as before.
