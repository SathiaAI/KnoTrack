...
implemented as of 2026-09-19 — idempotent one-way sync of a track to a GitHub
Issue / Linear Issue — and **offline-verified**; `kt_sync_to_github` has also
been dogfooded against a real repo, and `kt_sync_to_linear`'s one remaining
gate is a real-workspace dogfood. The one remaining stub is `kt_check_drift`,
which returns a well-formed empty scan (real drift heuristics land in T6).
